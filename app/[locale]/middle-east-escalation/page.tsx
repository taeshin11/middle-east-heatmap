...
export default async function MiddleEastEscalationPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sorted.map((c: any) => <CountryRiskCard key={c.id} country={c} locale={locale} />)}
      </div>
    </div>
  )
}
