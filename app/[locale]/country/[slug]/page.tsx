...
export default function CountryDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { slug } = use(params)
  const [country, setCountry] = useState<Country | null>(null)
  const [incidents, setIncidents] = useState<Incident[]>([])

  useEffect(() => {
    fetch('/data/countries.json').then(r => r.json()).then((cs: Country[]) => setCountry(cs.find(c => c.slug === slug) || null))
    fetch('/data/incidents.json').then(r => r.json()).then((incs: Incident[]) => setIncidents(incs.filter(i => i.country === slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()))))
  }, [slug])
